...
import { formatDate, isNowInRange } from '../../../utils/date';
...
const filteredTasks = computed(() => {
  if (!selectedDate.value) return tasks.value.filter((task) => isNowInRange(task.dateStart, task.dateEnd));

  return tasks.value.filter((task) => isNowInRange(task.dateStart, task.dateEnd, selectedDate.value));
});
...
<template>
  ...
  <section class="shadow-sm px-3 py-3 rounded-xl mt-3 dark:border dark:border-gray-200">
    <div class="flex justify-between items-center py-2">
      ...
      <div class="flex items-center gap-3">
        ...
        <button @click="isOpen = true" class="w-6 h-6 flex items-center justify-center bg-accent rounded-lg">
