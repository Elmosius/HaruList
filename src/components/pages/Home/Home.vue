...
const filteredTasks = computed(() =>
  tasks.value.filter((task) => isNowInRange(task.dateStart, task.dateEnd)),
);
</script>
...
<template>
  ...
  <section class="mt-5">
    <div class="flex justify-between items-center">
      ...
      <RouterLink to="/tasks" class="text-xs text-accent mt-1 font-bold"
        >View All</RouterLink
      >
    </div>
    ...
    <div>
      ...
      <div class="flex justify-center">
        <button
          @click="isOpen = true"
          class="bg-secondary dark:border dark:border-b-gray-200 text-primary px-6 py-2 rounded-xl mt-5 w-full md:w-1/2"
        >
          + Add New Task
        </button>
      </div>
